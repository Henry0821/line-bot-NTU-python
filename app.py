...
# 2. ngrok 有正常啟動(免費版的ngrok每次網址都是隨機的)
# 啟動應用程式指令
# ./ ngrok http 5001

# 3. ngrok 的網址是否有正確對應到 Line Webhook
...
def handle_message(event):
    with ApiClient(configuration) as api_client:
        ...
        if user_msg in faq:
            ...
            report = f"{user_msg}的匯率\n買價: {buy} \n賣價: {sell} \n資料來源:臺灣銀行匯率牌價公告，正式價格以臺灣銀行網站公告為準。"
            ...
            completion = client.chat.completions.create(
                ...
                messages=[
                    # System prompt
                    {"role": "system", "content": "你是一個機器人助理，你可以友善的回覆使用者詢問的問題。如果你認為使用者說的話跟「選單」或「menu」相關意思有關係，請直接回傳「menu」即可，其餘問題請回答使用者。"},
                    # User prompt
                    {"role": "user", "content": user_msg}
                ]
            )
            ai_reply_msg = completion.choices[0].message.content
            if ai_reply_msg == "menu":
                bot_msg = menu
            else:
